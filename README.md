...
- 导出：
  ...
  - `实验结果-实验增补表316.md`
问题
为什么\lambda是小数，为什么不是整数，后面3个数值是怎么定义的
如何生成编排方案？
每一步是都使ultilty function最大，不是贪心吗？你怎么使得全局最优？或者说你使用的a-star算法就能保证全局最优吗?
Cost怎么估计？怎么知道要耗多少token?而且token和latency是一码事吗？是否需要拆开？还是2者等价？
如何量化uncertainty这个指标
如何量化redundancy这个指标？就是和之前3个数值怎么定义是一样的。
为什么workflow是写死的一个pipeline?不是其他写死的pipeline?如果你找一个垃圾的pipeline对比？不是没意义？或者找多个workflow对比？
我们这个policy为什么优于threshold?从设计和实验2个角度说，到底哪里好了。
